# Layering Virtual Try-On — original upstream README

Preserved verbatim from [ChuenFung/Layering-Virtual-Try-On](https://github.com/ChuenFung/Layering-Virtual-Try-On).

> **Note:** the `pip install -e ./diffusers` step below does not work — the
> source tree shadows the install. See [`TROUBLESHOOTING.md`](./TROUBLESHOOTING.md).
> `environment.yml` is also a conda spec pinned to **linux-aarch64**. Use
> `runpod_setup.sh` instead.
